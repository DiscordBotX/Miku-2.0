...
class Egglord extends Client {
		...
		super({
			...
			presence: {
				...
				activities: [{
					name: 'my mention',
					type: 'LISTENING',
					url: 'https://dsc.gg/infinity-support',
				}],
			},
		});
	...
	*/
	SetActivity(type, array = []) {
		...
		try {
			let j = 0;
			setInterval(() => this.user.setActivity(`${this.Activity[j++ % this.Activity.length]}`, { type: type, url: 'https://dsc.gg/infinity-support' }), 10000);
		} catch (e) {
			console.log(e);
		}
	}
